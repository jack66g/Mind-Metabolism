...
class UnifiedMindSystem(nn.Module):
    def __init__(self, hidden_dim):
        ...
        self.persona_text = "\n".join(core_traits)
        # [修改点] 移除了硬编码的“休眠”，完全交由大模型动态生成或依靠张量碰撞
        ...
        self.alpha_residual = nn.Parameter(torch.tensor(0.9))  #原来的值是0.1
        ...
        self.knw_buffers = [] # [新增 - 小乐协议] 每一块矩阵专属的“错题本(Buffer)”
    ...
    def defrag_knw_expert(self, expert_idx, base_optimizer):
        ...
        for past_text in replay_batch: 
            ...
            if not (torch.isnan(loss) or torch.isinf(loss)):
                loss.backward()
                # 此时 lock=0，apply_knw_gradient_mask 将不会遮蔽任何旧区，全矩阵开放对齐
...
def save_mind(mind, path):
    torch.save({
        ...
        'knw_buffers': mind.knw_buffers # [新增] 持久化错题本
    }, path)
...
def load_mind(mind, path):
    ...
    mind.knw_buffers = cp.get('knw_buffers', [[] for _ in range(mind.knw_active_experts)]) # [新增] 加载错题本
...
# ==========================================
# 4. 核心心跳守护线程 (纯净版抽卡与上帝视角指令)
# ==========================================
def autonomous_heartbeat(mind):
    global SYSTEM_STATE, AUTONOMOUS_ACTIVE, LAST_AUTO_MSG_TIME
    
    while True:
        time.sleep(10) # 每10秒苏醒一次 (一个Tick)
        
        # 如果未开启自主开关，继续睡
        if not AUTONOMOUS_ACTIVE:
            continue
            
        # [并发结界保护]：如果处于训练、聊天或正在生成状态，强制挂起，绝对不打扰/抢显存！
...
def main():
    ...
    heartbeat_thread = threading.Thread(target=autonomous_heartbeat, args=(mind,), daemon=True)
    heartbeat_thread.start()

    while True:
        print("\n" + "="*80 + "\n1. [离线学泛化] | 2. [响应] | 3. [自发引擎开关] | 4. [刻录硬知识] | 5. [调整活跃度] | exit\n" + "="*80)
        ...
        if cmd == '1':
            ...
            # 一旦用户选择互动，立刻锁死状态机，后台休眠
            SYSTEM_STATE = "USER_BUSY" 
            with torch.no_grad():
                # 加入退出后门指令
                text = input("[用户问询 / 输入 q 退出当前话题]: ")
                
                if text.strip().lower() in ['q', '滚', '退下', '结束']:
                    SYSTEM_STATE = "IDLE"
                    LAST_AUTO_MSG_TIME = time.time()
                    print("🔇 已强制结束话题，AI 回归安静挂机状态。")
                    continue
                ...
                # [修改点：将上下文完美打包，彻底解决金鱼记忆 Bug]
                mind._update_memory(H_base, f"用户对我说：{text}\n我的回答是：{response_text}", raw_Q)

            # 互动结束，AI把球抛回给你，进入专属等待状态！
            LAST_AUTO_MSG_TIME = time.time()
            SYSTEM_STATE = "WAITING_FOR_REPLY"
            ...
            if AUTONOMOUS_ACTIVE:
                print("🟢 [系统提示]: AI 自主意识已开启！系统将根据多巴胺阈值在后台自发运转。")
                SYSTEM_STATE = "IDLE" 
            else:
                print("🔴 [系统提示]: AI 自主意识已关闭！后台线程休眠，回归被动响应模式。")
                ...
                for idx, text in enumerate(lines):
                    try:
                        ...
                        if mode == "SPAWN": 
                            ...
                            mind.knw_buffers.append([text]) # 新房子配新错题本
                            
                        elif mode == "EXPAND":
                            # 1. 扩容前先触发小乐协议，重算对齐特征！
                            mind.defrag_knw_expert(knw_e_idx, optimizer)
                            # 2. 物理拼接新矩阵
                            mind.expand_knw_expert(knw_e_idx)
                            # 3. 刺杀旧优化器，重置动量 (防止 Shape Mismatch 闪退)
                            optimizer = get_optimizer(mind)
                            # 4. 新知识记入错题本
                            mind.knw_buffers[knw_e_idx].append(text)
                            
                        else:
                            # DROP 或正常追加微调时，也记入错题本备用
                            if knw_e_idx != -1 and len(mind.knw_buffers) > knw_e_idx:
                                mind.knw_buffers[knw_e_idx].append(text)
                        # ==================================================
            ...
            if val_str.isdigit():
                ...
                print(f"   ► 理论后台驱动基准: 每 {mind.cycle_seconds} 秒")
